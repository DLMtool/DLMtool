---
title: "DLMtool: Data-Limited Methods Toolkit (v2.XX.X)"
...
## New Additions to this Version (add V number)
...
Spawning Potential Ratio (SPR) based MPs (e.g. Prince et al. 2015).
...
You can automatically detect the number of threads using `detectCores()`. Ie type `sfInit(parallel=T,detectCores())`. Currently I cannot get a vignette to build with a cluster so these lines are not included anymore. In practice you should use cluster computing however.
...
** Example of TradePlot and Performance Metrics here **
...
```{r, fig.width=7, fig.height=3.5, out.width='0.8\\linewidth', out.height='0.4\\linewidth'}
summary(ourReefFish)
```
...
** Check performance of methods here **
