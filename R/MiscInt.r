...
getFMSY2 <- function(x, M_ageArray, hs, Mat_age, Wt_age, R0, V, retA, maxage, nyears, 
    ...
    opt <- optimize(projOpt_cpp, log(c(0.001, 5)),
                    Mc = M_ageArray[x, ,nyears], hc = hs[x], Mac = Mat_age[x, ], 
                    Wac = Wt_age[x, , nyears], R0c = R0[x], Vc = V[x, ,nyears], 
                    retAc=retA[x,,nyears], nyears = nyears, maxage = maxage, movc = mov[x, , ], 
                    Spat_targc = Spat_targ[x], SRrelc = SRrel[x], aRc = aR[x, ], bRc = bR[x, ], 
                    proyears = proyears, Control=Control)
