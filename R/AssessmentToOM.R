...
# --------- iSCAM (Martell 2014) functions ------------


# --------- SS3 (Methot 2012) functions ---------------
...
#' @author T. Carruthers
#' @export LinInterp
LinInterp<-function(x,y,xlev){
  close<-which.min((x-xlev)^2)
...
SS2DLM<-function(SSdir,nsim=48,proyears=50,Name=NULL,Source="No source provided",Author="No author provided",printstats=F,verbose=T){
  
  message("-- Using package r4ss to extract data from SS file structure --")
  ...
  nseas<-1/replist$seasduration[1]
  ...
  V[,,1:nyears]<-rep(sumF[,3],each=nsim)
  ...
  OM@L5<-rep(LinInterp(Vuln,Len_age,0.05),2)                            # not used if V is in cpars
  ...
  recdevs<-replist$recruit$dev[(nrecs-nyears+1):nrecs]
  #recdevs<-replist[length(replist$recruit$dev)-nyears)]
  #recdevs[is.na(recdevs)]<-0
  OM@Perr<-rep(sd(recdevs),2) # uniform range is a point estimate from assessment MLE
  OM@AC<-rep(acf(recdevs)$acf[2,1,1],2)
  
  Perr<-matrix(rnorm(nsim*nyears,rep(recdevs,each=nsim),0.2),nrow=nsim) # generate a bunch of simulations with uncertainty
  procsd<-apply(Perr,1,sd)
  Perr<-exp(Perr)
  ...
  OM@Iobs<-rep(sd((replist$cpue$Obs-replist$cpue$Exp)/replist$cpue$Exp),2)
