...
```{r, echo=FALSE}
par(mfrow=c(1,3))
hist2(Pars$hs, col=plotPars$col, axes=plotPars$axes, main="Steepness (h)", breaks=plotPars$breaks)
abline(v=Pars$hs[its], col=1:nsamp, lwd=plotPars$lwd)
axis(side=1)  

hist2(Pars$procsd, col=plotPars$col, axes=plotPars$axes, main="Recruitment process error (Perr)", breaks=plotPars$breaks)
abline(v=Pars$procsd[its], col=1:nsamp, lwd=plotPars$lwd)
axis(side=1) 

hist2(Pars$AC, col=plotPars$col, axes=plotPars$axes, main="Auto-correlation (AC)", breaks=plotPars$breaks)
...
matplot(years, t(Pars$Perr_y[its,(maxage):ncol(Pars$Perr_y)]), type="l", bty="l", main="Rec Devs by Year", 
        lwd=plotPars$lwd, lty=1, ylab="Rec Devs", xlab="Years")
