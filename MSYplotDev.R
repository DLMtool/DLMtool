...
MSYFuns <- function(OM, sim=1, yr=1, silent=FALSE) {
  ...
  DF2 <- data.frame(Ages=1:maxage, M_at_Age=M_at_Age, Wt_at_Age=Wt_at_Age,
                    V_at_Age=V_at_Age, Mat_at_Age=Mat_at_Age)
  
  DF2 <- tidyr::gather(DF2, "key", "value", 2:5)
  
  ggplot2::ggplot(DF2 %>% dplyr::filter(key %in% c("Mat_at_Age", "V_at_Age")), 
                  ggplot2::aes(x=Ages, y=value, color=key)) + ggplot2::geom_line() +
    ggplot2::theme_classic() +
    ggplot2::labs(x="Age (year)", y="Probability")
  
  Ages <- 1:maxage
  plot(Ages, Mat_at_Age)
  lines(Ages, V_at_Age)
  plot(Ages, M_at_Age)
  plot(Ages, Wt_at_Age)
